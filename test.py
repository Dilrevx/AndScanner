...
def rom_extractor(file_path: str, rom_brand: str = None):
    """
    ...
    if rom_brand == 'zte':
        ...
        for root, dirs, files in os.walk(path_extract):
            for d in dirs:
                os.rename(os.path.join(root, d), os.path.join(
                    root, 'ZTE_Rom_'+str(time.time())))

        extracted = ROMExtractor(path_extract).extract()

        if os.popen(f'find {path_extract} -name "system.img.extracted"').read() == '':
            path_img = os.popen(
                f'find {path_extract} -name "system.img"').read()
            ...
            os.system(work_path+'/romanalyzer_extractor/tools/extfstools/ext2rd ' +
                      str(path_img)[:-1]+' ./:'+str(path_img)[:-1]+'.extracted')
        ...
        os.system(work_path+'/romanalyzer_extractor/tools/huawei_erofs/lpunpack ' +
                  str(extracted)+'/UPDATE.APP.extracted/SUPER.img.ext4 '+str(extracted)+'/tmp')
        os.system(work_path+'/romanalyzer_extractor/tools/huawei_erofs/erofsUnpackKt_x64 ' +
                  str(extracted)+'/tmp/system.img '+str(extracted)+'/sys')
    ...
    for root, dirs, files in os.walk(targetDir):
        for file_str in files:
            if file_str.endswith('.apk'):
                ...
                command_str = 'python ./static/androguard-3.3.6/main.py -i ' + \
                    path_str + ' -r ' + report_path
