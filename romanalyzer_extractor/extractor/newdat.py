...
class NewDatExtractor(Extractor):
    ...
    def extract(self):
        '''
        tools/sdat2img/sdat2img
        '''
        self.log.debug("New.dat extract: {}".format(self.target))
        workdir = self.target.parents[0]

        transfer_list = workdir / \
            "{}".format(self.target.name.replace('.new.dat', '.transfer.list'))

        if not transfer_list.exists():
            self.log.warn("cannot unpack {} because lack of {}".format(
                self.target, transfer_list))
            return None

        output_system_img = workdir / \
            "{}".format(self.target.name.replace('.new.dat', '.img'))
