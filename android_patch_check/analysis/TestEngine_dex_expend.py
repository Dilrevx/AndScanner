...
class TestEngine(object):
    ...
    def loadTestSuites(self):
        ...
        #/*modify*/ 对资源引用路径的修改
        #allTestSuites = json.load(open("/home/dell/tmp_ls/PatchSher/assets/allTestSuites.json"))
        allTestSuites = json.load(open("/data/mcl_workspace/android_patch_check_2303/assets/allTestSuites.json"))
        ...
        basicTestChunks = [
            #"/home/dell/tmp_ls/PatchSher/" + url.replace("https://snoopsnitch-api.srlabs.de", "assets")
            "/data/mcl_workspace/android_patch_check_2303/" + url.replace("https://snoopsnitch-api.srlabs.de", "assets")
    ...
    def getBasicTestResultByUUID(self, uuid):
        ...
        return self.executeBasicTest(test,uuid) if test else None
    ...
    def executeBasicTest(self, test, uuid):
        #/*modify*/主要是新增5个逻辑，DEX相关的4个，以及1个设备特征的逻辑。有可能需要对引用路径进行修改。
        ...
        if testType.startswith("DEX_"):
            dex_tool_path="/data/tmp_hou/andpatchchecker/android_patch_check/assets/basic_test.jar"
            temp_dic={
                "uuid":uuid,
                "class":test.get("class") if "class" in test.keys() else "",
                "dexPath":test.get("dexPath") if "dexPath" in test.keys() else "",
                "method":test.get("method") if "method" in test.keys() else "",
                "signature":test.get("signature") if "signature" in test.keys() else "",
                "testType":test.get("testType")
                }
            file_path=str(self._localFirmwareRoot)+'/'+temp_dic.get('dexPath')[temp_dic.get('dexPath').find(':')+2:]
            exec_order1="timeout 60s java -jar "+dex_tool_path+" '"+temp_dic.get("uuid")+"' '"+temp_dic.get("class")+"' '"+\
                                temp_dic.get("dexPath")+"' '"+temp_dic.get("method")+"' '"+temp_dic.get("signature")+\
                                "' '"+temp_dic.get("testType")+"' '"+file_path+"'"
            print(exec_order1)
        ...
        elif testType == "DEX_IS_VALID":
            r=os.popen(exec_order1)
            result=r.read().strip()
            return True if result=='true' else False
        elif testType == "DEX_CONTAINS_CLASS":
            r=os.popen(exec_order1)
            result=r.read().strip()
            return True if result=='true' else False
        elif testType == "DEX_CLASS_CONTAINS_METHOD":
            r=os.popen(exec_order1)
            result=r.read().strip()
            return True if result=='true' else False
        elif testType == "DEX_METHOD_SIGNATURE":
            r=os.popen(exec_order1)
            result=r.read().strip()
            return True if result=='true' else False
        elif testType == "DEVICE_HAS_FEATURE":
            #目前仅适配对android.hardware.nfc的检测
            #因为检测逻辑暂时没有对其他特征的处理
            so_path=str(self._localFirmwareRoot)+'/system/lib/'
            so_path=os.path.abspath(so_path)
            if os.path.isdir(so_path)==False:
                pass
            else:
                so_files=os.listdir(so_path)
                for f in so_files:
                    if f.startswith('android.hardware.nfc'):
                        return True
        
            so_path=str(self._localFirmwareRoot)+'/system/lib64/'
            so_path=os.path.abspath(so_path)
            if os.path.isdir(so_path)==False:
                pass
            else:
